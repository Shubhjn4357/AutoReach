      ...
      <div className="w-full lg:w-80 shrink-0 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-md flex flex-col overflow-hidden max-h-[calc(100vh-180px)] lg:max-h-none">
        ...
        <div className="p-4 border-b border-[var(--color-border)] flex flex-col gap-3 bg-black/5">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {(
              ["ALL", "NEW", "CONTACTED", "QUALIFIED", "WON", "LOST"] as const
            ).map((stage) => (
              <button
                key={stage}
            ...
            filteredLeads.map((lead) => (
              ...
              >
                ...
                <div className="flex justify-between items-center text-[10px]">
                  <span
                    className={`px-1.5 py-0.5 rounded-sm font-bold text-[8px] ${
                      lead.status === "WON"
        ...
        {selectedLead ? (
          <div className="flex flex-col gap-6">
              ...
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    <span className="bg-[var(--color-primary)]/10 text-[var(--color-primary)] px-2 py-0.5 rounded-sm text-[9px] font-bold">
                      {selectedLead.status}
                    </span>
                  </div>
                </div>
            ...
            <div className="bg-[var(--color-bg)] border border-[var(--color-border)] p-4 rounded-md">
              <span className="text-[9px] text-[var(--color-text-muted)] uppercase tracking-wider font-bold block mb-1.5">
                Internal Notes
              </span>
            ...
            <div className="bg-[var(--color-bg)] border border-[var(--color-border)] p-4 rounded-md">
              <div className="flex justify-between items-center mb-3">
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <span className="text-[10px] text-[var(--color-text-primary)] uppercase tracking-wider font-bold font-sans">
              ...
              <div className="flex flex-col gap-2">
                {driveFiles.filter((f) => f.leadId === selectedLead.id)
                  .length === 0 ? (
                  <div className="text-[11px] text-[var(--color-text-muted)] italic py-3 text-center bg-black/10 rounded-md">
                    No documents uploaded yet. Click above to generate an
                    agreement document.
                  </div>
                ) : (
                  driveFiles
                    ...
                    .map((file) => (
                      <div
                        key={file.id}
                        className="flex justify-between items-center bg-black/20 p-2.5 rounded-md border border-[var(--color-border)]"
                      >
            ...
            <div className="bg-[var(--color-primary)]/5 border border-[var(--color-primary)]/20 p-5 rounded-md">
              ...
              {aiLoading && (
                <div className="py-8 flex flex-col items-center justify-center gap-2">
                  ...
                  />
                  <span className="text-[10px] text-[var(--color-text-secondary)] italic animate-pulse">
              ...
              {aiResult ? (
                <div className="flex flex-col gap-4">
                  ...
                  <div className="bg-[var(--color-bg)] border border-[var(--color-border)] p-4 rounded-md">
                    <span className="text-[9px] text-[var(--color-primary)] uppercase tracking-wider font-bold block mb-1">
                  ...
                  {aiResult.proposedQuickReply && (
                    <div className="bg-[var(--color-bg)] border border-[var(--color-border)] p-4 rounded-md">
                      <span className="text-[9px] text-[var(--color-accent)] uppercase tracking-wider font-bold block mb-1.5">
                ...
                <div className="py-4 text-center text-[11px] text-[var(--color-text-muted)] italic">
                  No audit active. Click 'Audit Opportunity' to trigger local
                  model evaluator and generate actions.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div>
              <h2 className="text-xl font-bold text-[var(--color-text-primary)] font-sans">
                Welcome, {user?.name || "User"}
              </h2>
              <p className="text-xs text-[var(--color-text-secondary)] mt-1">
                Here is a quick overview of your current CRM pipelines &
                background workers.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="bg-[var(--color-bg)] border border-[var(--color-border)] p-4 rounded-md flex flex-col gap-1.5 shadow-sm">
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  ...
                  <span className="text-[9px] text-[var(--color-success)] font-bold ml-1">
                    +12%
                  </span>
                </div>
              </div>
              ...
              <div className="bg-[var(--color-bg)] border border-[var(--color-border)] p-4 rounded-md flex flex-col gap-1.5 shadow-sm">
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-xl font-black text-[var(--color-success)]">
                    {metrics.winRate}%
                  </span>
            ...
            <div className="bg-[var(--color-bg)] border border-[var(--color-border)] p-5 rounded-md">
              ...
              <div className="h-2 w-full rounded-md bg-[var(--color-border)] overflow-hidden flex">
                {(
                  ...
                  const ratio =
                    leads.length > 0 ? (stageCount / leads.length) * 100 : 0;
                ...
                {(
                  ...
                  return (
                    <div key={stage} className="flex items-center gap-1.5">
                      <span
                        className={`w-2 h-2 rounded-full ${stage === "NEW" ? "bg-[var(--color-accent)]" : stage === "CONTACTED" ? "bg-[var(--color-secondary)]" : stage === "QUALIFIED" ? "bg-[var(--color-primary)]" : stage === "WON" ? "bg-[var(--color-success)]" : "bg-[var(--color-danger)]"}`}
                      />
                      <span>
                        {stage}:{" "}
                ...
                <div className="text-[11px] text-[var(--color-text-secondary)] flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      ...
                      <code className="bg-black/40 px-1 rounded-sm">
                        local.db
                      </code>
                    </span>
                  </div>
