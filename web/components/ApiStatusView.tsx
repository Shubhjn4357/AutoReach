    ...
    <div className="max-w-4xl mx-auto flex flex-col gap-6">
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] p-5 rounded-md">
        ...
        <p className="text-xs text-[var(--color-text-secondary)] mt-1">
          Live status, healthchecks, and metrics logs of the monorepo
          integrations and router gateways.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] p-5 rounded-md flex flex-col justify-between gap-4">
          <div className="flex justify-between items-start">
            <div className="min-w-0">
              <h3 className="text-xs font-bold text-[var(--color-text-primary)] uppercase tracking-wider">
                CRM Leads Database API
              </h3>
              <code className="text-[10px] text-[var(--color-text-muted)] block mt-1">
                GET /api/leads
              </code>
            </div>
            <span
              className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                apiHealth.leads.status === "healthy"
                  ? "bg-[var(--color-success)]"
                  : apiHealth.leads.status === "error"
                    ? "bg-[var(--color-danger)]"
                    : "bg-[var(--color-border)]"
              }`}
            />
          ...
          <div className="bg-black/25 p-3 rounded-md border border-[var(--color-border)]">
            <span className="text-[9px] text-[var(--color-text-muted)] uppercase tracking-wider font-bold block mb-1">
              Latency & Status
            </span>
            <div className="text-xs text-[var(--color-text-secondary)] flex items-center justify-between">
              <span>
                ...
                <strong className="text-[var(--color-text-primary)] capitalize">
                  {apiHealth.leads.status}
                </strong>
              </span>
              {apiHealth.leads.latency !== null && (
                <span>
                  Latency:{" "}
                  <strong className="text-[var(--color-text-primary)]">
                    {apiHealth.leads.latency}ms
                  </strong>
                </span>
              )}
            </div>
        ...
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] p-5 rounded-md flex flex-col justify-between gap-4">
          <div className="flex justify-between items-start">
            ...
            <span
              className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                apiHealth.drive.status === "healthy"
                  ? "bg-[var(--color-success)]"
                  : apiHealth.drive.status === "error"
                    ? "bg-[var(--color-danger)]"
                    : "bg-[var(--color-border)]"
              }`}
            />
          ...
          <div className="bg-black/25 p-3 rounded-md border border-[var(--color-border)]">
            ...
            <div className="text-xs text-[var(--color-text-secondary)] flex items-center justify-between">
              <span>
                Status:{" "}
                <strong className="text-[var(--color-text-primary)] capitalize">
                  {apiHealth.drive.status}
                </strong>
              </span>
              {apiHealth.drive.latency !== null && (
                <span>
                  Latency:{" "}
                  <strong className="text-[var(--color-text-primary)]">
                    {apiHealth.drive.latency}ms
                  </strong>
                </span>
              )}
            </div>
        ...
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] p-5 rounded-md flex flex-col justify-between gap-4">
          <div className="flex justify-between items-start">
            ...
            <span
              className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                apiHealth.sync.status === "healthy"
                  ? "bg-[var(--color-success)]"
                  : apiHealth.sync.status === "error"
                    ? "bg-[var(--color-danger)]"
                    : "bg-[var(--color-border)]"
              }`}
            />
          ...
          <div className="bg-black/25 p-3 rounded-md border border-[var(--color-border)]">
            ...
            <div className="text-xs text-[var(--color-text-secondary)] flex items-center justify-between">
              <span>
                Status:{" "}
                <strong className="text-[var(--color-text-primary)] capitalize">
                  {apiHealth.sync.status}
                </strong>
              </span>
              {apiHealth.sync.latency !== null && (
                <span>
                  Latency:{" "}
                  <strong className="text-[var(--color-text-primary)]">
                    {apiHealth.sync.latency}ms
                  </strong>
                </span>
              )}
            </div>
          ...
          <button
            onClick={() =>
              pingEndpoint("sync", "/api/sync", "POST", { operations: [] })
            }
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-md overflow-hidden shadow-sm">
        <div className="px-5 py-3.5 border-b border-[var(--color-border)] bg-black/5 flex items-center justify-between">
          ...
          <span className="text-[9px] bg-[var(--color-border)] text-[var(--color-text-secondary)] font-mono px-2 py-0.5 rounded-sm">
            Console JSON
          </span>
        </div>
        ...
        <div className="p-4 bg-black/45 text-[11px] font-mono text-[var(--color-text-secondary)] overflow-x-auto max-h-64 leading-relaxed">
          {Object.values(apiHealth).every((h) => h.status === "idle") ? (
            <div className="text-center italic py-6 text-[var(--color-text-muted)]">
              Diagnostic console is empty. Ping any gateway integration above to
