  ...
  try {
    ...
    if (!operations || !Array.isArray(operations)) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "BAD_REQUEST", message: "Missing operations" },
        },
        { status: 400 },
      );
    }
    ...
    for (const op of operations) {
      try {
        ...
        const targetDb =
          op.table === "leads" ? leadsInMemoryDb : tasksInMemoryDb;
        ...
        try {
          if (op.table === "leads") {
            if (op.operation === "CREATE" || op.operation === "UPDATE") {
              const createdAtVal = payload.createdAt
                ...
                : Date.now();
              const updatedAtVal = payload.updatedAt
          ...
          console.warn(
            `SQLite write failed for ${op.table} [${op.recordId}]:`,
            dbErr.message,
          );
        }
